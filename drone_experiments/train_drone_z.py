...
parser.add_argument('--pre_trained', type=str, default='weights/z/new2_model_epoch22_3.421_u1.522.pth',help='input your pretrained weight path if you want')
args = parser.parse_args()
print(args)

save_path = f'weights/z'
x_r = np.array([args.reference])
data_tmp = Data_Purify(args.dataset,x_r,p_max=10,u_max=5)
...
def test(model, test_loader,epoch):
    ...
    torch.save(model.state_dict(), os.path.join(
        save_path,'new2_model_epoch{}_{:.3f}_u{:.3f}.pth'.format(
...
for epoch in range(num_epochs):
    for inputs, targets in train_loader:
        optimizer.zero_grad()
        outputs = model(inputs.to(device))
        loss1 = 100*criterion(outputs[:,:3], targets[:,:3].to(device)) # P_loss
        loss2 = 500*criterion(outputs[:,3], targets[:,3].to(device)) # u_loss
        # tricks: 
        # 1. add more weights on u_loss, less time consumption
        # 2. keep above weights file, add more weights on P_loss, closer to reference
        # a reference ratio of u_loss:P_loss = 10:300 (lr=0.0001) --> 1000:200 (lr=0.000006)
        loss = loss1 + loss2
    loss.backward()
